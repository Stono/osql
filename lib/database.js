...
var MssqlDriver = function(options) {
  ...
  var procedure = {
    ...
    input: function(name, type, value){
      utils.isValidDataType(type);
      params.push({'type': 'input', 'name': name || '', 'dataType': type, 'value': value});
    },
    ...
    execute: function(name, callback){
      this.request()
        .then(function(request){
          utils.assignParameter(params, request);
          request.execute(name, function(err, recordSet, returnValue){
            params = [];
            callback(err, recordSet, returnValue);
          });
        });
    }    
  };
